...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	// using mkstream here if the stream doesnt exist we create new one .
	// notice here this process is like if (!streamExist) mkstream in node.js 
	// the mechanism of rust and nodejs is different as in node it will send 2 reqs but rust deal with both at once , and this costs less time . 
	// () : its like void return , _ in Result is for return err .
	// 0 : means that group will start with oldest msgs .
	...
	let reader_handle = tokio::spawn(async move { // spawn is foe giving imp task to other thread
		...
		loop {
			...
			// options which mean there will be some or none .
			// streamReadReply is a struct conains actual msg data (id, value , fields) .
				.xread_options(&[stream_name], &[">"], &options)
				// &[">"] : tells valkey to give new msgs only , rather than using (0, 0) it make sure msgs will not repeated to different users . 
				.await
			...
			if let Some(reply) = res {
				for stream_key in reply.keys {
					for stream_id in stream_key.ids {
						...
						// xack: make valkey to remove msg from pending list , this is better when other user recieve the msg
						// in case any crash happend this make sure msg will not be lost .
						if let Err(res) = res {
